...
{
    return put(data,dlen,0);
}

enum Sock2::SockRet Sock2::put(char *data,uint32_t dlen,int bflag)
{
    enum SockRet ret;

    ret = put_len(dlen,bflag);
    if (ret != READY) {
        return ret;
    }
...
enum Sock2::SockRet Sock2::put_len(uint32_t dlen,int bflag)
{
    unsigned short s;
    unsigned long l;
    char tbuf[128];
